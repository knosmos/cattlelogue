...
def build_cnn_data(year=2015, stride=4):
    """
    ...
    for i in range(0, features.shape[0] - PATCH_SIZE + 1, STRIDE):
        for j in range(0, features.shape[1] - PATCH_SIZE + 1, STRIDE):
            ...
            y.append(ground_truth[i + PATCH_SIZE // 2, j + PATCH_SIZE // 2] > 1)
...
def train_cnn_model(epochs, batch_size, learning_rate, step_size, gamma):
    """
    Train a CNN model for livestock density prediction.
    Args:
    ...
    """
    patches, ground_truth = build_cnn_data()
    print("Livestock data statistics:", calc_livestock_stats(ground_truth))
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = CNN(in_channels=patches[0][0].shape[-1], out_channels=1).to(device)
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            for batch_features, batch_ground_truth in val_loader:
                ...
                val_loss += loss.item()
                y_true = np.concatenate((y_true, batch_ground_truth.cpu().numpy()))
                y_pred = np.concatenate((y_pred, outputs.squeeze().cpu().numpy()))
